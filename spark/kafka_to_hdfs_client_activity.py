...
kafka_topic = "e_krylova_client_activity_info"
kafka_bootstrap_servers = "172.17.0.13:9092"

#------------------------------------------------------------------------------------------------------------------------------------
# Сохранение файла с информацией об активностях клиентов

# Пути для сохранения данных на HDFS clients_activity
# study_project_b
hdfs_output_path = "hdfs://172.17.0.23:8020/user/e.krylova/test2/activity/output_raw_json"  # Директория для временных файлов (JSON)
checkpoint_location = "hdfs://172.17.0.23:8020/user/e.krylova/test2/activity/checkpoint"  # Директория для checkpoint-файлов
final_output_path = "hdfs://172.17.0.23:8020/user/e.krylova/test2/activity/activity_csv_files"  # Конечный файл с объединенными данными в CSV

# Чтение данных из Kafka топик e_krylova_client_activity_info
df = spark.readStream \
    .format("kafka") \
...
if not query.isActive:
    ...
    # Шаг 2: Преобразование полей
    df_final = df_filtered.select(
        col("client_id").cast(DoubleType()).alias("client_id"),
        col("activity_date").cast(TimestampType()).alias("activity_date"),
        col("activity_type").alias("activity_type"),
        col("activity_location").alias("activity_location"),
